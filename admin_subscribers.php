...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $allowedUses = (int)($_POST['discount_uses_allowed'] ?? 0);

    if ($subscriberId > 0 && $allowedUses >= 1) {

        $updateStmt = $conn->prepare("
            UPDATE subscribers
            SET discount_uses_allowed = ?
            WHERE id = ?
        ");
    ...
    $redirectUrl = 'admin_subscribers.php';
...
if ($search !== '') {
    ...
    $subscriberStmt = $conn->prepare("
        SELECT
            ...
            s.discount_code,
            s.discount_percent,
            s.discount_uses_allowed,
            s.discount_uses_used,
            u.id AS user_id,
            u.name AS user_name
        FROM subscribers s
        LEFT JOIN users u
    ...
    $result = $subscriberStmt->get_result();
                        ...
                        <?php foreach ($subscribers as $subscriber): ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <form
                                        method="POST"
                                        action="admin_subscribers.php<?= $search !== '' ? '?search=' . urlencode($search) : '' ?>"
                                        class="discount-form"
                                    >
